...
import { useInsurances } from '../insurances.service';
...
export function InsurancesPage(){
  ...
  return <div className='space-y-4'>
    <div className='flex items-center justify-between'>
      <h1 className='text-xl font-semibold'>Assurances</h1>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild><Button>Ajouter</Button></DialogTrigger>
        <DialogContent className='max-w-3xl'><DialogHeader><DialogTitle>Nouvelle assurance</DialogTitle></DialogHeader><AddInsuranceForm onCreated={()=> setOpen(false)} /></DialogContent>
      </Dialog>
    </div>
    ...
    <DataTable columns={insuranceColumns} data={data || []} isLoading={isLoading || isTyping} totalRows={pagination?.total || 0} currentPage={pageIndex} pageSize={pageSize} onPageChange={setPageIndex} onPageSizeChange={(s)=> { setPageSize(s); setPageIndex(0);} } />
  </div>;
}
